...
export const makeRichTextFontStylesCSS = value => {
  return value
    .map(item => {
      ...
      const classNameDesktop = `.brz-tp-${id.toLowerCase()}`;
      const classNameDesktop2 = `.brz-tp-lg-${id.toLowerCase()}`;
      const classNameTablet = `.brz-tp-sm-${id.toLowerCase()}`;
      const classNameMobile = `.brz-tp-xs-${id.toLowerCase()}`;
