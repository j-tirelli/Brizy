...
import { getAuthors, getPosts, getTerms } from "visual/utils/api/index.wp";
...
export async function termsLoad(
  ...
  try {
    const terms = await getTerms({ include: valueParsed, abortSignal });
...
export async function termsSearch(
  ...
  try {
    const terms = await getTerms({ search, abortSignal });
