import { IS_PAGE, IS_CMS, IS_ARCHIVE } from "visual/utils/env";
...
const dynamic = [];
const dynamicCMS = [PostTitle, PostExcerpt, Posts];
const dynamicArchive = [Archive, PostTitle, PostExcerpt];

const config = (() => {
  const isUnknown = !IS_PAGE && !IS_ARCHIVE && !IS_STORY;

  if (IS_PAGE || isUnknown) {
    return {
      dynamic: IS_CMS ? dynamicCMS : dynamic,
      base,
      grid,
      social
    };
